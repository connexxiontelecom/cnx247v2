				...
				<div class="col-sm-12">
					<table class="table table-responsive invoice-table invoice-total">
						...
						<tr class="text-info">
							...
							<td>
								<hr>
								<strong class="text-primary">{{ $receipt->getCurrency->symbol ?? Auth::user()->tenant->currency->symbol}} {{number_format(($invoices->sum('total') + $invoiceBalance->sum('tax_value') / $receipt->exchange_rate / $receipt->exchange_rate) - ($payments->sum('payment')/$receipt->exchange_rate),2) }}</strong>
							</td>
						</tr>
						</tbody>
					</table>
